DOMAINS   							%Pemberian nama pada parameter adalah fungsi domains
...
PREDICATES							%predicates digunakan untuk mendklarasikan relasi    
	nondeterm orang(nama, umur, jender, pekerjaan)  	%relasi orang yaitu nama,umur, jender dan pekerjaan
	nondeterm selingkuh(nama, nama)				%relasi selingkuh yaitu nama dan nama
		terbunuh_dengan(nama, benda)   			%relasi terbunuh dengan yaitu nama dengan benda apa
		terbunuh(nama)   				%relasi tebunuh yaitu nama 
	nondeterm pembunuh(nama)   				%relasi predikat pembunuh yaitu parameter nama
		motif(alasan)   				%relasi predikat motif adalah parameter alasan 
		ternodai(nama, zat)   				%relasi predikat ternodai yaitu dengan parameter nama dan zat
		milik(nama, benda)   				%relasi predikat milik yaitu parameter nama dan benda
	nondeterm cara_kerja_mirip(benda, benda)		%relasi predikat cara  kerjanya mirip yaitubenda danbenda   
	nondeterm kemungkinan_milik(nama, benda)   		%relasi predikat kemungkinan milik yaitu parameter nama dan benda
	nondeterm dicurigai(nama) 				%relasi predikat dicurigai yaitu parameternama
 
/* * * Fakta-fakta tentang pembunuhan * * */ 
CLAUSES 							%clauses berisi fakta
  	orang(budi,55,m,tukang_kayu).   			%budi adalah tukang kayu yang berumur 55 dengan bergender m  
  	orang(aldi,25,m,pemain_sepak_bola).   			%aldi adalah pemain sepak bola yang berumur 55 dengan bergender m  
  	orang(joni,25,m,pencopet). 				%budi adalah tukang kayu yang berumur 55 dengan bergender m  
...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * */
   	...
   	dicurigai(X):-  					%X dicurigai jika maka
   		terbunuh_dengan(siti,Senjata) ,			%Siti dibunuh dengan senjata dan
...
* * * * * * * * * * * * * * * * * * * * * * * * * */ 
  	...
  	dicurigai(X):-  					%X dicurigai jika maka
  		...
  		orang(X,_,m,_),  				%X adalah orang tersebut yang bergender m dan
...
* * * * * * * * * * * * * * * * * * * * * * */ 
  	...
  	dicurigai(X):-  					%X dicurigai jika maka
  		motif(cemburu),  				%cemburu adalah motifnya dan
  		orang(X,_,f,_),  				%X adalah orang tersebut yang bergender f dan
